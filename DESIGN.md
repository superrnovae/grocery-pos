...
| Mise à jour automatique branchée sur un fournisseur générique `electron-builder` avec une URL provisoire                                            | Démontre l'intégration complète d'`electron-updater` de bout en bout (le chemin de code est réel et fonctionnel) ; une installation opérationnelle nécessite encore que l'URL provisoire pointe vers un véritable hébergeur de fichiers statiques ou GitHub Releases avant de pouvoir servir de vraies mises à jour. |
...
- **Les mises à jour automatiques** sont branchées de bout en bout sur une configuration réelle `electron-updater` + `electron-builder` (`publish: generic`), mais l'URL dans `electron-builder.yml` est provisoire (`https://updates.example.com/grocery-pos`). Il faudra la faire pointer vers un véritable hébergeur de fichiers statiques (ou basculer vers un fournisseur comme GitHub Releases) servant une version construite ainsi que son `latest.yml` généré, avant qu'une vérification de mise à jour puisse trouver quoi que ce soit.
